...
with tabs[1]:
    ...
    with left:
        ...
        ax.set_xlabel(kor_col(target_col), fontsize=9)
    ...
    if numeric_cols:
        st.markdown("#### 3. 이탈과 수치형 특성의 관계")
        corr_df = df[numeric_cols + [target_col]].corr(numeric_only=True)[target_col].drop(target_col).sort_values(key=abs, ascending=False)
        fig, ax = compact_fig(7.0, 3.2)
        corr_df.head(8).sort_values().plot(kind="barh", ax=ax, color="#f97316")
        ax.set_title("이탈과 상관관계가 큰 수치형 특성", fontsize=11)
        ax.set_xlabel("상관계수", fontsize=9)
        style_axis(ax)
        st.pyplot(fig, width='content')
    ...
    if model_name == "모델을 선택하세요":
        ...
        if has_grid_result:
            ...
            if st.button("모델 학습 실행", type="primary", key="manual_train_button"):
                ...
                proba = predict_probability(pipe, X_test)
                ...
                st.session_state["train_settings"] = {
                    ...
                    "numeric_scaling": "StandardScaler 고정 적용",
                }
                st.rerun()
